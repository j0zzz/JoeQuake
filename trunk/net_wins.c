...
// so we can run on Win 3.1, where there isn't necessarily Winsock)
	if (!(hInst = LoadLibrary("wsock32.dll")))
	{
	...
	if (!pWSAStartup || !pWSACleanup || !pWSAGetLastError || !psocket || !pioctlsocket || 
	    !psetsockopt || !precvfrom || !psendto || !pclosesocket || !pgethostname || 
	    !pgethostbyname || !pgethostbyaddr || !pgetsockname)
	{
	...
	{
//joe		Con_Printf ("WINS_Init: Unable to open control socket\n");
		if (--winsock_initialized == 0)
			pWSACleanup ();
		return -1;
	}
...
//joe	Con_Printf ("Winsock TCP/IP Initialized\n");
	tcpipAvailable = true;
