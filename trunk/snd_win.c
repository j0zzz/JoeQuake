...
sndinitstat SNDDMA_InitDirect (void)
{
	...
	while ((hresult = iDirectSoundCreate(NULL, &pDS, NULL)) != DS_OK)
	{
		...
		{
			Con_Printf ("DirectSoundCreate failure, hardware already in use\n");
			return SIS_NOTAVAIL;
		}
...
//			Con_Printf ("Using secondary sound buffer\n");
	...
	else
	{
		...
		pDSBuf = pDSPBuf;
//joe		Con_Printf ("Using primary sound buffer\n");
	}
...
qboolean SNDDMA_InitWav (void)
{
	...
	if ((hr = waveOutOpen((LPHWAVEOUT)&hWaveOut, WAVE_MAPPER, &format, 0, 0L, CALLBACK_NULL)) != MMSYSERR_NOERROR)
	{
		if (hr == MMSYSERR_ALLOCATED)
			Con_Printf ("waveOutOpen failed, hardware already in use\n");
		else
