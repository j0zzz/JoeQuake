...
static void S_UpdateFilter(filter_t *filter, int M, float f_c)
{
	if (filter->f_c != f_c || filter->M != M)
	{
		...
		if (!filter->memory || !filter->kernel)
			Sys_Error ("S_UpdateFilter: out of memory (%llu bytes)", (uint64_t)(filter->kernelsize * sizeof (float)));
...
static void S_ApplyFilter(filter_t *filter, int *data, int stride, int count)
{
	...
	input = (float *) Hunk_Alloc (inputsize);
...
void S_PaintChannels (int endtime)
{
	...
	while (paintedtime < endtime)
	{
		...
		{
			paintbuffer[i].left = bound(-32768 * 256, paintbuffer[i].left, 32767 * 256) / 2;
			paintbuffer[i].right = bound(-32768 * 256, paintbuffer[i].right, 32767 * 256) / 2;
		}

		// apply a lowpass filter
		if (s_khz.value == 11 && shm->speed == 44100)
		{
