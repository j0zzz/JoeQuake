...
****************************************************************************/
{
	...
	if (!(dc = MGL_createDisplayDC(npages)))
		return NULL;
...
void VID_InitFullDIB (HINSTANCE hInstance)
{
	...
	int	numlowresmodes, bpp, done, cstretch, istretch, mstretch;
	BOOL	stat;
...
char *VID_GetExtModeDescription (int mode)
{
	...
	pv = VID_GetModePtr (mode);
	if (modelist[mode].type == MS_FULLSCREEN)
		sprintf (pinfo,"%s fullscreen %s",pv->modedesc, MGL_modeDriverName(pv->modenum));
	else if (modelist[mode].type == MS_FULLDIB)
...
// empty while Quake starts up.
	...
	if (!(windc = MGL_createWindowedDC(mainwindow)))
		MGL_fatalError ("Unable to create Windowed DC!");

	if (!(dibdc = MGL_createMemoryDC(DIBWidth,DIBHeight,8,&pf)))
	...
	// Tonik: this is a workaround for the bug with garbaged
	// screen on Riva TNT
...
qboolean VID_SetFullscreenMode (int modenum)
{
	...
	if (!(mgldc = createDisplayDC(modelist[modenum].stretched || (int)vid_nopageflip.value)))
		return false;
...
{
	int	i, bestmatch, bestmatchmetric, basenummodes, t, dr, dg, db;
	byte	*ptmp;
...
LONG WINAPI MainWndProc (HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	// Tonik: this is a workaround for the bug with garbaged screen on Riva TNT
